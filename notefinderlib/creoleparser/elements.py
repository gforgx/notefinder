...
class Highlight(InlineElement):
    def __init__(self, tag, token, child_tags):
        super(Highlight,self).__init__(tag,token , child_tags)
        self.regexp = re.compile(self.re_string())

    def _build(self,mo,element_store):
        return bldr.tag.__getattr__(self.tag)(style='background:yellow')(fragmentize(mo.group(1), self.child_tags, element_store))

class Eval(InlineElement):
    def __init__(self, tag, token, child_tags):
        super(Eval,self).__init__(tag,token , child_tags)
        self.regexp = re.compile(self.re_string())

    def _build(self,mo,element_store):
        try:
            result = str(eval(mo.group(1)))
